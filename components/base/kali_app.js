...
export class KaliApp extends Component {
    ...
    handleContextMenu = (e) => {
        // Only show context menu for user-created folders
        if (this.props.id && this.props.id.startsWith('new-folder-')) {
            e.preventDefault();
            e.stopPropagation();
            if (this.props.onFolderContextMenu) {
                this.props.onFolderContextMenu(e, this.props.id);
            }
        }
    }
